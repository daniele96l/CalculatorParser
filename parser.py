...
class Lexer:
    ...
    def make_tokens(self):
        ...
        print(tokens)
        tokens.append(Token(TT_EOF, pos_start=self.pos)) #il token di fine stringa
        return tokens, None
...
class BinOpNode:
    def __init__(self, left_node, op_tok, right_node):
        ...
        print(self.left_node + " " + self.right_node)
...
#class UnaryOpNode:
  #  def __init__(self, op_tok, node):  SE TOLGO QUESTO RIESCO AD EVITARE IL A++A
   #     self.op_tok = op_tok
    #    self.node = node
...
class Parser:
    ...
    def parse(self):
        ...
        if not res.error and self.current_tok.type != TT_EOF:
            return res.failure(InvalidSyntaxError(  #questo da errore se
    ...
    def factor(self):   #the grammar how a factor works
        res = ParseResult()
        ...
        if tok.type in (TT_PLUS, TT_MINUS): #
            ...
            return res.success(UnaryOpNode(tok, factor))  #credo quindi il primo nodo, che sarà il nodo con il + o -
            #unary node non deve esistere , è malvagio


        elif tok.type in (variableName, TT_VAR_A, TT_VAR_B, TT_VAR_C, TT_INT, TT_FLOAT): #devo fare l'advance anche per le lettere c:
            res.register(self.advance())
            return res.success(NumberNode(tok))  #creo quindi il nodo del fattore che sarà il numero o la variabile

        elif tok.type == TT_LPAREN: #check sulla correttezza delle parentesi
            res.register(self.advance())
